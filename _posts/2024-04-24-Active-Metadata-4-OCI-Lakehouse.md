...
In this first example, OCI Function uses the OCI Python SDK to set the value of the Custom Properties of a Data Catalog entity.
...
In this second example, OCI Function uses the OCI Python SDK to set the value of the Custom Properties of a Data Catalog folder.
...
> Note: When an OCI Event invokes an OCI Function, you cannot specify input parameters. Then in these examples, I used the OCI Function described in the above paragraph and another one very similar (named *oci-datacatalog-update-customproperties-folder-ko-python*), with the only difference that it updates the Folder Custom Properties with the proper value in the event of an error (*Last Update Not Available*).
...
For this article, I created two OCI Data Integration RestAPI tasks which invoke the example of OCI Function that updates an entity Custom Properties.
