...
- **Autonomous Data Warehouse**, with a data model composed of four tables for curated data: *REVENUE_TARGET*, *CUSTOMERS_TARGET*,*EMPLOYEES_WEST_MIDWEST*, *EMPLOYEES_WEST_MIDWEST*:
  ![Fig.2: OCI Lakehouse ADW data models](/data-organon/images/2024-04-24-Active-Metadata-4-OCI-Lakehouse/adw-data-model.png)
- **OCI Object Storage**, with a bucket that stores the files used to load curated data into ADW data model:
  ![Fig.3: OCI Lakehouse Object Storage Bucket](/data-organon/images/2024-04-24-Active-Metadata-4-OCI-Lakehouse/DI-bucket.png)
- **OCI Data Integration**, with a workspace which includes task, data loads and pipelines that load data into the ADW data model. As an example, the picture below shows the main pipeline with the steps to load the ADW target tables:
...
For this article, I create a custom property that gets updated automatically with the outcomes of the most recent data update process. This will require the following steps:
...
- Adding Tasks in the OCI Data Integration pipeline to invoke the OCI Function.
- As alternative to automatically update the property, using OCI Data Integration events to invoke the OCI Function.
...
![Fig.8: Custom Properties creation](/data-organon/images/2024-04-24-Active-Metadata-4-OCI-Lakehouse/custom-property-creation.png)

Since it's enabled for all the data catalog types, I can start using it. For example, I set its value as *Not Known* for the *Customer* data entity:
...
As examples, I created OCI Functions to update the Custom Properties of a Data Catalog Entity and of a Folder (which, in the case of an Oracle DB asset, corresponds to a DB schema).

#### OCI Function to update Entity Custom Properties

This OCI Function uses the OCI Python SDK to set the value of the Custom Properties of a Data Catalog entity.
...
This OCI Function uses the OCI Python SDK to set the value of the Custom Properties of a Data Catalog folder.
...
Then I create two events rule, one that executes an OCI Function in case the OCI DI Task *Load DWH Pipeline Task* runs successfully and the other one that execute another OCI Function in case the same task ends with errors.

> Note: When an OCI Event invokes an OCI Function, you cannot specify input parameters. Then in these examples, I used the OCI Function described in the [above paragraph](###-oci-function-to-update-folder-custom-properties) and another one very similar (named *oci-datacatalog-update-customproperties-folder-ko-python*), with the only difference that it updates the Folder Custom Properties with the proper value in the event of an error (*Last Update Not Available*).
...
If I run the pipeline task, and it completes successfully:
...
I can see the Custom Properties updated accordingly:
...
Then, as a test, I rerun the pipeline making it fail intentionally (I stopped the ADW instance):
...
For this article, I created two OCI Data Integration RestAPI tasks which invoke the OCI Function that updates an entity Custom Properties (see [OCI Function to update Entity Custom Properties](####oci-function-to-update-entity-custom-properties)).
As example, below you can see the RestAPI task built for the successful updates of the *Customer* entity (which is the ADW table *CUSTOMER_TARGET*):
